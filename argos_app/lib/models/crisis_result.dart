// Models for the HAVEN backend /api/v1/demo/analyze response.
...
class DispatchDecision {
  ...
  final String priority; // 'primary' | 'secondary'
  ...
  factory DispatchDecision.fromJson(Map<String, dynamic> json) {
    return DispatchDecision(
      ...
      equipmentToBring: (json['equipment_to_bring'] as List<dynamic>? ?? [])
          .map((e) => e.toString())
          .toList(),
...
class ExternalEscalation {
  ...
  final String service; // 'fire_dept' | 'ambulance' | 'police' | 'none'
  final String reason;
  ...
  factory ExternalEscalation.fromJson(Map<String, dynamic> json) {
    return ExternalEscalation(
      ...
      autoCallInMinutes: (json['auto_call_in_minutes'] as num? ?? 0).toInt(),
    );
  }
}
    ...
    return CrisisResult(
      ...
      dispatchDecisions:
          (json['dispatch_decisions'] as List<dynamic>? ?? [])
              .map((e) => DispatchDecision.fromJson(e as Map<String, dynamic>))
              .toList(),
